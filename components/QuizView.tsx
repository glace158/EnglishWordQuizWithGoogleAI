...
const QuizView: React.FC<Props> = ({ quiz, voices, selectedVoice, setSelectedVoice, wordsCount }) => {
  ...
  useEffect(() => {
    if (quiz.state.currentWord && !quiz.state.isSubmitted && quiz.quizMode !== QuizMode.WORD_MATCH) {
  ...
  const getQuestionText = () => {
    ...
    if (quiz.quizMode === QuizMode.AI_DICTATION) return ''; // AI Hear 모드에서는 텍스트 미노출
  ...
  const handleRetry = () => {
    quiz.setState((prev: any) => ({ ...prev, userInput: '', isSubmitted: false, isCorrect: false, aiFeedback: '', showHint: false }));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
        {[ 
          ...
          { mode: QuizMode.EXAMPLE_GAP, title: 'Sentence' }, 
          { mode: QuizMode.WORD_MATCH, title: 'Match' },
          { mode: QuizMode.AI_SENTENCE_GEN, title: 'AI Draft' },
          { mode: QuizMode.AI_DICTATION, title: 'AI Hear' }
        ].map(item => (
          <button 
            key={item.mode} 
            onClick={() => { 
              quiz.setQuizMode(item.mode); 
              quiz.getNextWord(item.mode); // 즉시 새로운 모드로 단어 로드 및 음성 재생 트리거
            }} 
            className={`group relative p-3 rounded-2xl border transition-all duration-300 ${quiz.quizMode === item.mode ? 'bg-[#1a1c24] border-purple-500/50 shadow-lg' : 'bg-[#0e1117] border-slate-800 hover:border-slate-700'}`}
          >
            <div className={`font-black text-[11px] md:text-sm ${quiz.quizMode === item.mode ? 'text-purple-400' : 'text-slate-400'}`}>{item.title}</div>
            {quiz.quizMode === item.mode && (
              <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-6 h-1 bg-purple-500 rounded-full shadow-[0_0_12px_rgba(168,85,247,0.8)] transition-all duration-300"></div>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-4 items-center justify-between bg-[#0e1117] p-5 rounded-2xl border border-slate-800 shadow-xl">
        <div className="flex bg-[#080a0f] p-1 rounded-lg">
          ...
          <button onClick={() => quiz.setSelectionMode(SelectionMode.RANDOM)} className={`px-3 py-1.5 text-xs font-bold rounded-md ${quiz.selectionMode === SelectionMode.RANDOM ? 'bg-indigo-600 text-white shadow-[0_0_10px_rgba(79,70,229,0.5)]' : 'text-slate-500'}`} title="망각 곡선 가중치 기반 복습 모드">Weighted (SRS)</button>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end leading-none border-r border-slate-800 pr-4">
            <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Daily AI Quota</span>
            <span className={`text-xs font-bold ${quiz.remainingQuota < 100 ? 'text-rose-400 animate-pulse' : 'text-emerald-400'}`}>{quiz.remainingQuota.toLocaleString()} Left</span>
          </div>
          <select value={selectedVoice} onChange={(e) => setSelectedVoice(e.target.value)} className="bg-[#080a0f] border border-slate-800 text-xs rounded-xl p-2.5 text-slate-300 outline-none max-w-[150px] md:max-w-[200px]">
      ...
      {/* Main Quiz Area */}
      <div className="bg-[#11141b] p-6 md:p-12 rounded-[2.5rem] shadow-2xl border border-slate-800/50 min-h-[480px] flex flex-col relative overflow-hidden">
        {wordsCount === 0 ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center w-full text-center space-y-8">
            <button 
              ...
              className={`absolute top-8 right-8 w-12 h-12 rounded-2xl flex items-center justify-center transition-all z-20 ${quiz.state.currentWord?.isMastered ? 'bg-emerald-500 shadow-lg shadow-emerald-500/20 text-white' : 'bg-slate-900/50 border border-slate-800 text-slate-500 hover:border-slate-600 hover:text-slate-300'}`}
            ...
            <div className="bg-[#1a1c24] p-10 rounded-3xl border border-slate-800 w-full max-w-2xl shadow-inner relative flex flex-col items-center justify-center min-h-[160px]">
                ...
                <div className="flex flex-col items-center gap-6">
                  {quiz.quizMode === QuizMode.AI_DICTATION && (
                    <button 
                      ...
                      className="w-24 h-24 rounded-full bg-indigo-600 text-white flex items-center justify-center shadow-2xl shadow-indigo-500/40 hover:bg-indigo-500 transition-all active:scale-90 border-4 border-white/10"
                    >
                      <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 2.929a1 1 0 011.414 0A9.972 9.972 0 0119 10a9.972 9.972 0 01-2.929 7.071 1 1 0 01-1.414-1.414A7.971 7.971 0 0017 10c0-2.21-.894-4.208-2.343-5.657a1 1 0 010-1.414zm-2.829 2.828a1 1 0 011.414 0A5.982 5.982 0 0115 10a5.982 5.982 0 01-1.414 4.243 1 1 0 01-1.414-1.414A3.982 3.982 0 0013 10a3.982 3.982 0 00-1.414-2.828a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                    </button>
                  )}
                  <h2 className={`font-bold text-slate-100 flex items-center justify-center gap-4 ${
                    quiz.quizMode === QuizMode.EXAMPLE_GAP || quiz.quizMode === QuizMode.AI_SENTENCE_GEN || quiz.quizMode === QuizMode.AI_DICTATION
                      ? 'text-2xl italic text-slate-300 leading-relaxed px-4' 
                      : (quiz.quizMode === QuizMode.KR_TO_EN ? 'text-3xl md:text-5xl tracking-tight' : 'text-5xl md:text-7xl tracking-tighter')
                  }`}>
                    {getQuestionText()}
                    {quiz.quizMode === QuizMode.KR_TO_EN && !quiz.state.isSubmitted && (
                      <button onClick={playWord} className="p-3 bg-slate-800 rounded-2xl text-slate-400 hover:text-indigo-400 transition-all hover:bg-slate-700 active:scale-90 shadow-lg" title="Pronounce">
                        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M14,3.23V5.29C16.89,6.15 19,8.83 19,12C19,15.17 16.89,17.85 14,18.71V20.77C18,19.86 21,16.28 21,12C21,7.72 18,4.14 14,3.23M16.5,12C16.5,10.23 15.5,8.71 14,7.97V16.02C15.5,15.29 16.5,13.77 16.5,12M3,9V15H7L12,20V4L7,9H3Z" /></svg>
                      </button>
                    )}
                  </h2>
                </div>
            ...
            <div className="w-full max-w-xl space-y-6">
              {/* Perfectly Centered Input with Outer Mic Button */}
              <div className="flex items-center gap-2 md:gap-4 w-full justify-center">
                {/* Spacer to balance the mic button on the right and keep input centered */}
                {!quiz.state.isSubmitted && !quiz.state.isAiLoading && <div className="w-12 md:w-16 hidden sm:block pointer-events-none opacity-0 shrink-0"></div>}
                ...
                <input 
                  ...
                  className={`flex-1 text-center py-4 px-4 md:py-6 md:px-6 text-xl md:text-4xl font-bold bg-[#080a0f] border-b-4 outline-none rounded-2xl transition-all duration-300 ${
              ...
              {quiz.state.isSubmitted ? (
                <div className="flex flex-col gap-4 animate-in zoom-in-95 duration-300">
                  <div className="px-8 py-6 rounded-[2.5rem] font-black shadow-2xl border-2 border-white/10 flex flex-col gap-3" style={{ backgroundColor: quiz.state.isCorrect ? '#10b981' : '#f43f5e' }}>
                    <div className="text-white text-2xl flex items-center justify-center gap-2">
                    ...
                    {quiz.state.aiFeedback && (
                      <div className="text-white/95 text-sm font-bold bg-black/20 p-5 rounded-2xl whitespace-pre-wrap text-left leading-relaxed">
                        {quiz.state.aiFeedback}
                      </div>
                    )}
                  </div>
